...
st.markdown(
    ...
    <style>
      ...
      .acc-header { display:flex; align-items:center; gap:16px; border-bottom:1px solid #E6E6E6; padding:10px 0 14px 0; }
...
# Layout general + ejes profesionales
fig.update_layout(
    height=540,
    hovermode="x unified",
    ...
    legend=dict(orientation="h", y=1.12, x=1.0, xanchor="right", font=dict(size=11), tracegroupgap=8),
    margin=dict(l=30, r=10, t=90, b=30),
)

# Rejillas suaves + ejes (ISA-101): líneas, ticks afuera, márgenes auto y separación títulos
for c in range(1, 6):
    fig.update_xaxes(
