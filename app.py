...
# Objetivo: animar la respuesta del modelo ante cambios de alimentación
# - 2 días, paso 30 min (96 frames)
# - Un tambor (T1) para simplificar
# - Recalcula: P80, dosificación (Ácido/Refino/Agua en kg/t) y Hum_out (%)
# - Muestra mezcla UGM_A/UGM_B como % (proxy de UGM alimentada)
# - Animación con Plotly frames (Play/Pause) y trazas iniciales visibles
# --------------------------------------------------------------
...
<div style='background:{C['panel']}; padding:10px; border-radius:12px;'>
  ...
  <p style='color:{C['silver']}; margin:0;'>Respuesta del modelo: P80, dosificación (kg/t), Humedad_out (%) y % UGM_B</p>
</div>
...
# Interpretación: share_UGM_B = blend_signal, share_UGM_A = 1 - blend_signal
share_B_pct = 100.0 * blend_signal

# Leyes base (dos extremos) que se mezclan (UGM_A vs UGM_B)
...
# Trazas base (iniciales con el primer punto para que se vea algo antes del Play)
fig.add_trace(go.Scatter(name="Ácido (kg/t)", x=T["timestamp"].iloc[:1], y=T["Acid_kgpt"].iloc[:1], mode="lines",
                         line=dict(color=C["magenta"], width=2)), row=1, col=1, secondary_y=False)
fig.add_trace(go.Scatter(name="Refino (kg/t)", x=T["timestamp"].iloc[:1], y=T["Refino_kgpt"].iloc[:1], mode="lines",
                         line=dict(color=C["blue"], width=2)), row=1, col=1, secondary_y=False)
fig.add_trace(go.Scatter(name="Agua (kg/t)", x=T["timestamp"].iloc[:1], y=T["Agua_kgpt"].iloc[:1], mode="lines",
                         line=dict(color=C["green"], width=2)), row=1, col=1, secondary_y=False)
fig.add_trace(go.Scatter(name="Humedad_out (%)", x=T["timestamp"].iloc[:1], y=T["Hum_out_pct"].iloc[:1], mode="lines",
                         line=dict(color=C["silver"], width=2, dash="dot")), row=1, col=1, secondary_y=True)
# %UGM_B como segunda señal en eje % para mostrar UGM alimentada (proxy)
fig.add_trace(go.Scatter(name="% UGM_B", x=T["timestamp"].iloc[:1], y=T["%UGM_B"].iloc[:1], mode="lines",
                         line=dict(color="#FF66FF", width=1)), row=1, col=1, secondary_y=True)
...
for i in range(len(T)):
    fr = go.Frame(
        ...
        data=[
            ...
            go.Scatter(x=T["timestamp"].iloc[:i+1], y=T["%UGM_B"].iloc[:i+1]),
        ]
    )
    frames.append(fr)
...
fig.update_layout(
    ...
    title_text="T1 — Setpoints (kg/t), Humedad (%) y % UGM_B",
