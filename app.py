...
st.markdown(
    ...
    <style>
      ...
      .acc-header { display:flex; align-items:center; justify-content:space-between; border-bottom:1px solid #E6E6E6; padding:8px 0 14px 0; }
      .acc-title { font-weight:700; font-size:22px; color:#111; }
...
with col_logo:
    logo = st.file_uploader("Logo (PNG) opcional", type=["png"], key=key("logo") )
    if logo:
    ...
    st.markdown(
        ...
        <div class='acc-header'>
          <div>
            ...
            <div class='acc-sub'>ISA‑101 style • Mezcla UGM → P80/finos → Leyes → Dosificación → Humedad</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
# ------------------------------
fig = make_subplots(rows=1, cols=5, shared_xaxes=False,
